...
class facturar(FormData):
    ...
    def __init__(self, hcaller, name=""):
        ...
        """
        # Conexión a datos
        self.strsql='SELECT * FROM facturas'
        ...
        self.setIcon('img/contabOtro32.png')
        ...
        self.statusbar.createTip(self.btnimprimir, "Imprimir factura seleccionada [Alt-I]")
    ...
    def declaraControls(self):
        ...
        tk.Label(self.cabecera, text="NUMERO:").grid(row=0,column=0, sticky=tk.E, padx=3, pady=3)
        self.idfac=tk.Label(self.cabecera, bg='white',borderwidth=1,
                           relief="solid", width=8,anchor=tk.E)
        self.idfac.grid(row=0, column=1, sticky=tk.W, padx=3, pady=3)
        tk.Label(self.cabecera, text="Fecha:").grid(row=1,column=0,sticky=tk.E,padx=3, pady=3)
        self.fecfac=tk.Label(self.cabecera, bg='white',borderwidth=1,
                           relief="solid", width=10, anchor=tk.W)
        self.fecfac.grid(row=1,column=1,sticky=tk.W, padx=3, pady=3)
        tk.Label(self.cabecera, text="CLIENTE").grid(row=0, column=3,padx=3,pady=3,sticky=tk.E)
        self.nomcli=tk.Label(self.cabecera, bg='white',borderwidth=1,
                           relief="solid", width=40,anchor=tk.W)
        self.nomcli.grid(row=0, column=4,padx=3,pady=3,sticky=tk.EW)
        self.nifcli=tk.Label(self.cabecera, bg='white',borderwidth=1,
                           relief="solid", width=10,anchor=tk.W)
        self.nifcli.grid(row=0, column=5,padx=3,pady=3,sticky=tk.W)
        self.dircli=tk.Label(self.cabecera, bg='white',borderwidth=1,
                           relief="solid", width=40,anchor=tk.W)
        self.dircli.grid(row=1, column=3,columnspan=3,padx=3,pady=3,sticky=tk.EW)
        self.cabecera.columnconfigure(4, weight=1)
        self.cabecera.pack(side=tk.TOP, fill='x', padx=5, pady=10, ipadx=5)
        # Area detalle
        self.detalle=tk.Frame(self)
        tk.Label(self.detalle, text="Importe Neto:").grid(row=0,column=0, columnspan=2, sticky=tk.E, padx=3, pady=3)
        self.impfac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=9,anchor=tk.E)
        self.impfac.grid(row=0, column=2,padx=3,pady=3,sticky=tk.EW)
        tk.Label(self.detalle, text="% IVA:").grid(row=1,column=0,sticky=tk.E, padx=3, pady=3)
        self.ivafac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=5,anchor=tk.E)
        self.ivafac.grid(row=1, column=1,padx=3,pady=3,sticky=tk.EW)
        self.impiva=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=9,anchor=tk.E)
        self.impiva.grid(row=1, column=2,padx=3,pady=3,sticky=tk.EW)
        tk.Label(self.detalle, text="% IRPF:").grid(row=2,column=0,sticky=tk.E, padx=3, pady=3)
        self.irpfac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=5,anchor=tk.E)
        self.irpfac.grid(row=2, column=1,padx=3,pady=3,sticky=tk.EW)
        self.impirpf=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=9,anchor=tk.E)
        self.impirpf.grid(row=2, column=2,padx=3,pady=3,sticky=tk.EW)
        tk.Label(self.detalle, text="TOTAL:").grid(row=3,column=0, columnspan=2, sticky=tk.E, padx=3, pady=3)
        self.totalfac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=9,anchor=tk.E)
        self.totalfac.grid(row=3, column=2,padx=3,pady=3,sticky=tk.EW)
        tk.Label(self.detalle, text="Concepto:").grid(row=0,column=3, sticky=tk.E, padx=3, pady=3)
        self.cnpfac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=40,anchor=tk.W)
        self.cnpfac.grid(row=0, column=4, padx=3,pady=3,sticky=tk.EW)
        tk.Label(self.detalle, text="F. Pago:").grid(row=1,column=3, sticky=tk.E, padx=3, pady=3)
        self.fpafac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=40,anchor=tk.W)
        self.fpafac.grid(row=1, column=4, padx=3,pady=3,sticky=tk.EW)
        tk.Label(self.detalle, text="Tipo Ingreso:").grid(row=3,column=3, sticky=tk.E, padx=3, pady=3)
        self.tpifac=tk.Label(self.detalle, bg='white',borderwidth=1,
                           relief="solid", width=40,anchor=tk.W)
        self.tpifac.grid(row=3, column=4, padx=3,pady=3,sticky=tk.EW)
        
        self.detalle.columnconfigure(4,weight=1)
        self.detalle.pack(side=tk.BOTTOM,fill=tk.X, pady=10, padx=5)
        
        # Cambio el numero de factura a negrita
        fteWidget=self.idfac.cget('font')
        fuente = tkfont.Font(family=fteWidget)
        #familia=fuente.actual()['family']
        tamano = fuente.actual()["size"]
        # peso = fuente.actual()['weight']
        self.idfac['font']=[fuente,tamano,'bold']

    
    def showdata(self,*args):
        self.registro = self.rst[self.current.get()]
        self.factura=self.registro['id']
        self.idfac['text']=self.factura
        self.fecfac['text']=datetime.datetime.strptime(self.registro['fecfac'][:10],'%Y-%m-%d').date().strftime('%d-%m-%y')
        idcli=self.rst[self.current.get()]['clifac']
        rscli=db.consultar("SELECT * FROM clientes WHERE id={}".format(idcli),con=self.cnn)
        self.nomcli['text']=rscli[0]['empcli']
        self.nifcli['text']=rscli[0]['nifcli']
        direccion=f"{rscli[0]['dircli'].strip()} {rscli[0]['discli']} {rscli[0]['pobcli'].strip()}({rscli[0]['prvcli'].strip()})"
        self.dircli['text']=direccion
        neto=self.rst[self.current.get()]['impfac']
        self.impfac['text']="{:6.2f}".format(self.rst[self.current.get()]['impfac'])
        iva=self.rst[self.current.get()]['ivafac']
        irpf=self.rst[self.current.get()]['irpfac']
        impiva=round((neto/100)*iva,2)
        impirpf=round((neto/100)*irpf,2)
        self.ivafac['text']="{:7.2f}".format(self.rst[self.current.get()]['ivafac'])
        self.irpfac['text']="{:7.2f}".format(self.rst[self.current.get()]['irpfac'])
        self.impiva['text']="{:7.2f}".format(impiva)
        self.impirpf['text']="{:7.2f}".format(impirpf)
        total=neto+impiva-impirpf
        self.totalfac['text']="{:7.2f}".format(total)
        self.cnpfac['text']=self.rst[self.current.get()]['cnpfac']
        self.fpafac['text']=self.rst[self.current.get()]['fpafac']
        tipoing=self.rst[self.current.get()]['tpifac']
        rstpi=db.consultar("SELECT * FROM tiposing WHERE id={}".format(tipoing),con=self.cnn)
        self.tpifac['text']=rstpi[0]['nomtip']
    ...
    def imprimeDetalle(self, *args):
        ...
        dlg.dialogo(self, "IMPRESION", "Se imprimirá la factura "+self.idfac['text']).showmodal()
