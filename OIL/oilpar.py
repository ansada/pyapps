...
class frmparam(Form):
    ...
    # Eventos
    # def on_closing(self):
    #     # Como si hubiésemos pulsado cancel
    #     # o cerrado la ventana con X
    #     # o Alt+F4
    #     self.destroy()
    #     self.hcaller.show()
    #     self.close()
    ...
    def on_savepar(self, *args):
        self.prop.setProperty('clifacoil', self.idCliFac())
        self.prop.saveProperties()
        messagebox.showinfo(
            "Parametros OIL", "Guardados datos modificados", parent=self)
        self.closer()
    ...
    def showFields(self, event=""):
        ...
        cliente = self.nomCliFac(self.prop.getProperty("clifacoil", 0))
        if cliente == 0:
